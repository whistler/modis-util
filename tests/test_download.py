...
class TestDownload(unittest.TestCase):
    ...
    def test_scene_prefix(self):
        s3_prefix = download._get_s3_prefix('MCD43A4.A2017006.h21v11.006.2017018074804')
        assert s3_prefix == 'MCD43A4.006/21/11/2017006/'

    def test_list_files(self):
        files = download.list_files('MCD43A4.A2017006.h21v11.006.2017018074804')
    ...
    def test_download_file(self):
        ...
        download.download_file(s3_key, self.TMP_DIR)
    ...
    def test_filter_files_include(self):
        ...
        filtered_files = download._filter_files(files, ['_a', '_c.txt'], None)
        assert filtered_files == ['file_a.txt', 'file_c.txt']

    def test_filter_files_exclude(self):
        files = ['file_a.txt', 'file_b.txt', 'file_c.txt']
        filtered_files = download._filter_files(files, None, ['_a', '_c.txt'])
        assert filtered_files == ['file_b.txt']

    def test_filter_files_noop(self):
        files = ['file_a.txt', 'file_b.txt', 'file_c.txt']
        filtered_files = download._filter_files(files, None, None)
        assert filtered_files == files

    def test_download_scene(self):
        download.download('MCD43A4.A2017006.h21v11.006.2017018074804', self.TMP_DIR, include=['_meta'])
